#pragma once
#include "../SPIDevice.hpp"

const unsigned char font[480] = {};

class Nokia5110 {
    char dc;
    SPIDevice spi;
public:
    Nokia5110() {
        spi = SPIDevice(5, 0, true, 100000);
        dc = 7;
        GPIO(dc, GPIO::OUTPUT).setLevel(false);
        spi.transfer(0x21);
        spi.transfer(0xB0);
        spi.transfer(0x04);
        spi.transfer(0x14);
        spi.transfer(0x20);
        spi.transfer(0x0C);
        clear();
    }

    void clear() {
        GPIO(dc).setLevel(true);
        for(int i = 0; i < 504; i++) spi.transfer(0xAA);
        setCursor(0,0);
    }

    void setCursor(int row, int column) {
        GPIO(dc, GPIO::OUTPUT).setLevel(false);
        spi.transfer(0x80|(column*6));
        spi.transfer(0x40|row);
    }

    void setContrast(char contrast) {
        GPIO(dc, GPIO::OUTPUT).setLevel(false);
        spi.transfer(0x21);
        spi.transfer(contrast);
        spi.transfer(0x20);
    }

    void write(int row, int column, char* text) {
        setCursor(row,column);
        GPIO(dc, GPIO::OUTPUT).setLevel(true);
        for(int i = 0; text[i] != 0; i++) {
            int data = text[i] - 0x20;
            spi.transfer(font[(5*data) + 0]);
            spi.transfer(font[(5*data) + 1]);
            spi.transfer(font[(5*data) + 2]);
            spi.transfer(font[(5*data) + 3]);
            spi.transfer(font[(5*data) + 4]);
            spi.transfer(0x00);
        }
    }

    void writeInt(int row, int column, int number, int len) {
        setCursor(row,column);
        GPIO(dc, GPIO::OUTPUT).setLevel(true);
        char data[len];
        bool neg = number < 0;
        if(neg) number *= -1;
        int buf = 1;
        for(int i = 0; i < len; i++) {
            if((number/buf > 0 || i == 0) && (i < (len-1) || !neg)) {
                data[(len - i) - 1] = ((number/buf)%10) + 0x10;
            } else {
                data[(len - i) - 1] = neg?0x0D:0x00;
                neg = false;
            }
            buf*=10;
        } for(int i = 0; i < (len); i++) {
            spi.transfer(font[(5*data[i]) + 0]);
            spi.transfer(font[(5*data[i]) + 1]);
            spi.transfer(font[(5*data[i]) + 2]);
            spi.transfer(font[(5*data[i]) + 3]);
            spi.transfer(font[(5*data[i]) + 4]);
            spi.transfer(0x00);
        }
    }
};